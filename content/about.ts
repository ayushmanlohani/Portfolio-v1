/**
 * ┌───────────────────────────────────────────────────────────────┐
 * │  THE ABOUT ME TEXT. EDIT THIS FILE.                           │
 * │  Nothing here is code. Change the words, save, and the page   │
 * │  updates itself — the dev server reloads on its own.          │
 * └───────────────────────────────────────────────────────────────┘
 *
 * LINKS
 *   Write them as [the words people see](the address).
 *
 *     Along the way, I've built [Unitwise](https://unitwise.example.com).
 *
 *   An address of "#" means "no link yet" — those words come out bold
 *   instead, so the page never carries a link that goes nowhere. Swap the
 *   "#" for a real address and it becomes a link with no other change.
 *
 * APOSTROPHES
 *   Type them normally. ' and " are fine. No HTML codes needed.
 *
 * PARAGRAPHS
 *   One entry in the list below is one paragraph. Add or remove entries
 *   freely — keep each one inside its backticks and end it with a comma.
 */

export const NAME = "Ayushman Lohani";

export const PARAGRAPHS = [
  `I'm a final-year CSE (AI) student at the University of Lucknow, currently working as a Research Assistant.`,

  `I've built things that people actually use, watched them break, and then spent multiple nights fixing them. Right now, I'm working on a lightweight computer vision model with YOLO 26n, while exploring AI agents, RAG, and whatever else catches my attention.`,

  `Along the way, I've built [Unitwise](https://unitwise-weld.vercel.app/) and [RBI Sentinel](https://rbi-sentinel.streamlit.app).`,

  `Chekout my [Github](https://github.com/ayushmanlohani) and [Leetcode](https://leetcode.com/u/ayushmanlohani/).`,
];
